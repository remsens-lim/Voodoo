...
if __name__ == '__main__':
    ...
    nf = kwargs['mkdir'] if 'mkdir' in kwargs else 'TEST_desp/'
    ...
    train_path = join(data_path, f'20181127-20190927-{fn}-allclasses-{dc}-12ch2pol.zarr')

    # setting device on GPU if available, else CPU
    PT_settings.update({'dev': torch.device(f'cuda:{iGPU}' if torch.cuda.is_available() else 'cpu')})

    # make new dir
    h.change_dir(join(pt_models_path, nf))
    ...
    if len(md) > 0:
        ...
        m_name = f'{Vnet_label}-{dc}-fn{fn}-eps{EPOCHS}-bs{BATCH_SIZE}-bl{bl}.pt'
        ts_kwargs = {
            'balance': balance,
            'garbage': None,
        ...
        if addmore:
            train_path2 = join(data_path, f'20181127-20190927-{fn+5}-allclasses-{dc}-12ch2pol.zarr')
        ...
        fig, ax = PLT.create_acc_loss_graph(stat)
    ...
    for key, val in case_list.items():
        ...
        f, ax = TM.VoodooNet.create_quicklook(V_prediction['CLASS'])
