...
def get_filter_mask(bsc, depol, **kwargs):
    """
    ...
    Returns:
        mask (array): int2D - containing corrected backscatter and depolarization matrices, NOTE: The mask of the input container is updated!
    ...
    min_bsc_thresh = kwargs['bsc_thresh'][0] if 'bsc_thresh' in kwargs else 1.e-7
    ...
    factor_thresh = kwargs['thresh_fac'] if 'thresh_fac' in kwargs else 1.0e-1
    ...
    for iT, v in enumerate(depol['ts']):
        attbsc_corrected = bsc_cpy[iT, :]
        depol_corrected = depol_cpy[iT, :]
        ...
        # initialize lists
        idx_scl_start, idx_scl_end, idx_bsc_max = [], [], 0
        att_flag = False
        if len(idx_scl_end_2) > 0:
            ...
            for idx1 in idx_scl_end_2:
                ...
                if n_bins_250m + 5 >= idx1 - idx0 > 5:
                    ...
                    flags[iT, old_idx1:idx1] = 1
                    if np.mean(depol_corrected[idx0:idx1]) < 0.15:
        ...
        elif idx_bsc_max > 0 and not att_flag:
            flags[iT, idx_bsc_max:] = 3

    return flags


def apply_filter_mask(mask, data, mask_attenuated=False, mask_liq=False):
    """
    Applying the filter mask of the multiple scattering filter routine.

    Args:
        mask (array): multiple scattering mask
        data (dict): container of depolarization values

    Keyword Args:
        **mask_attenuated (bool): set values beyond attenuation to nan

    Returns:
        depol_out (float): 2D - containing corrected backscatter and depolarization matrices, NOTE: The mask of the input container is updated!
    """

    val_out = data['var'].copy()
    mask_out = data['mask'].copy()
    if mask_attenuated:
        val_out[mask == 3] = np.nan
        mask_out[mask == 3] = True

    if mask_liq:
        val_out[mask == 2] = np.minimum(val_out[mask == 2], 2.5e-2)
...
def apply_filter(bsc, depol, **kwargs):
    ...
    min_bsc_thresh = kwargs['bsc_thresh'][0] if 'bsc_thresh' in kwargs else 1.e-5
    max_bsc_thresh = kwargs['bsc_thresh'][1] if 'bsc_thresh' in kwargs else 1.e-3
    min_depol_thresh = kwargs['depol_thresh'][0] if 'depol_thresh' in kwargs else 0.0
    max_depol_thresh = kwargs['depol_thresh'][1] if 'depol_thresh' in kwargs else 0.5
    n_smoothing_ts = kwargs['n_smoothing_ts'] if 'n_smoothing_ts' in kwargs else 0
        ...
        if len(idx_scl_end_2) > 0:
            ...
            for idx1 in idx_scl_end_2:
                ...
                if n_bins_250m + 5 >= idx1 - idx0 > 5:
                    ...
                    new_depol = np.minimum(np.linspace(1.5e-2, 1.e-2, n_rg_liq_dpl), depol_corrected[idx0:idx1])
...
if __name__ == '__main__':
    ...
    end_dt = datetime.datetime(2019, 8, 1, 17, 59, 55)
    #
    ...
    #begin_dt = datetime.datetime(2019, 12, 20, 0, 0, 5)
    #end_dt = datetime.datetime(2019, 12, 20, 23, 5, 55)
    ...
    plot_settings = {'fig_size': fig_sz, 'range_interval': plot_range, 'contour': contour, 'rg_converter': True}
    ...
    if plot_profiles or plot_timeheight:
        attbsc_1064['name'] = 'attbsc 1064\n'
        fig, ax = pyLARDA.Transformations.plot_timeheight(attbsc_1064, z_converter="log",  **plot_settings)
        ...
        attbsc_532['name'] = 'attbsc 1064\n'
        fig, ax = pyLARDA.Transformations.plot_timeheight(attbsc_532,  z_converter="log", **plot_settings)
        fig.savefig(f'polly_bsc532_original_{begin_dt:%Y%m%d_%H%M%S_}{end_dt:%H%M%S}.png', dpi=300)
        print(f'polly_bsc532_original_{begin_dt:%Y%m%d_%H%M%S_}{end_dt:%H%M%S}.png')


        fig, _ = pyLARDA.Transformations.plot_timeheight(voldepol_532, **plot_settings)
    ...
    #
        ...
        fig, axVel = pyLARDA.Transformations.plot_timeheight(LIMRAD94_VEL, title=False, **plot_settings)
    ...
    #
    #
    status_flags = get_filter_mask(attbsc_1064, voldepol_532)

    print(f'Number of non-liquid pixel = {np.sum(status_flags==1)}')
    print(f'Number of liquid pixel     = {np.sum(status_flags==2)}')
    print(f'Number of attenuated pixel = {np.sum(status_flags==3)}')
    voldepol_532['var'], voldepol_532['mask'] = apply_filter_mask(status_flags, voldepol_532, mask_attenuated=True, mask_liq=True)
    attbsc_1064['var'],  attbsc_1064['mask']  = apply_filter_mask(status_flags, attbsc_1064, mask_attenuated=True)
    attbsc_532['var'],   attbsc_532['mask']   = apply_filter_mask(status_flags, attbsc_532, mask_attenuated=True)
    ...
    if plot_timeheight:
        ...
        fig, _ = pyLARDA.Transformations.plot_timeheight(attbsc_1064, z_converter="log", **plot_settings)
        ...
        # this is the last valid index
        attbsc_532['colormap'] = 'cloudnet_jet'
        # attbsc_1064['var_lims'] = [1e-7, 1e-3]
        fig, _ = pyLARDA.Transformations.plot_timeheight(attbsc_532, z_converter="log", **plot_settings)
        # ax.plot(dt_list, attbsc_1064['rg'][bsc_max], color='k', linestyle='--', label=r'thresh1')
        name = f'polly_bsc532_corrected_{begin_dt:%Y%m%d_%H%M%S_}{end_dt:%H%M%S}.png'
        fig.savefig(name, dpi=300)
        print(name)
        #
    ...
    if plot_trainingset:
        ...
        intp_settings = {'new_time': LIMRAD94_ZE['ts'], 'new_range': LIMRAD94_ZE['rg'], 'method': 'nearest'}

        bsc_ip = pyLARDA.Transformations.interpolate2d(attbsc_1064, **intp_settings)
        depol_ip = pyLARDA.Transformations.interpolate2d(voldepol_532, **intp_settings)

        bsc_ip['mask'] = np.logical_or(bsc_ip['mask'], LIMRAD94_ZE['mask'])
        depol_ip['mask'] = np.logical_or(depol_ip['mask'], LIMRAD94_ZE['mask'])

        fig, _ = pyLARDA.Transformations.plot_timeheight(bsc_ip, z_converter='log', **plot_settings)
        fig.tight_layout()
        fig_name = 'attbsc_trainingset_' + f'{begin_dt:%Y%m%d_%H%M%S_}{end_dt:%H%M%S}.png'
        fig.savefig(fig_name, dpi=300)
        print(f' Save figure :: {fig_name}')

        fig, _ = pyLARDA.Transformations.plot_timeheight(depol_ip, plot_settings)
        fig.tight_layout()
        ...
        fig, _ = pyLARDA.Transformations.plot_scatter(depol_ip, bsc_ip,
                                                      fig_size=[7, 7], x_lim=[1.e-7, 0.5], y_lim=[-7, -2.5],
